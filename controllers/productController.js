...
export const createProductController = async (req, res) => {
  try {
    ...
    switch (true) {
      ...
      case image && image.size > 2000000: //1MB ~ 1e6 bytes
        return res.status(500).send({
          error: "Image is required and its size should be less then 2MB",
        });
    }
...
export const productFiltersController = async (req, res) => {
  try {
    ...
    res.status(400).send({
      success: false,
      message: "Error while applying the product filter",
      error,
    });
  }
};

// return count of the product count
...
// return the product list in that page
...
export const productPageListController = async (req, res) => {
  try {
    ...
    res.status(400).send({
      success: false,
      message: "Error while fetching the list of products in that page",
      error,
    });
  }
};
    ...
    let newTransaction = gateway.transaction.sale(
      {
        ...
        merchantAccountId:"my_new_merchant",
        options: {
